...
#pragma region IPictureViewer implementation
    ...
    ResultCode OIV::LoadFile(void* buffer, std::size_t size, char* extension, OIV_CMD_LoadFile_Flags flags,
                             ImageHandle& handle)
    {
        ...
        //      ImageLoaderFlags loadFlags = ((flags & OIV_CMD_LoadFile_Flags::OnlyRegisteredExtension) != 0) ?
        //      ImageLoaderFlags::OnlyRegisteredExtension : ImageLoaderFlags::None; ImageResult loadResult =
        //      fImageLoader.Load(static_cast<const std::byte*>(buffer), size, extension,ImageLoadFlags::None,
        //      loadFlags, image);
    ...
    IMCodec::ImageSharedPtr OIV::ApplyExifRotation(IMCodec::ImageSharedPtr image) const
    {
        ...
        /*IMUtil::AxisAlignedTransform transform;
        transform.rotation = static_cast<IMUtil::AxisAlignedRotation>(ResolveExifRotation(image->Get
        GetMetaData().exifData.orientation)); transform.flip = IMUtil::AxisAlignedFlip::None; return
        IMUtil::ImageUtil::Transform(transform, image);*/
    }
    ...
    ResultCode OIV::CreateText(const OIV_CMD_CreateText_Request& request, OIV_CMD_CreateText_Response& response)
    {
        ...
        //        *reinterpret_cast<LLUtils::Color*>(&createParams.backgroundColor) = *reinterpret_cast<const
        //        LLUtils::Color*>(&request.backgroundColor); createParams.fontPath = fontPath; createParams.fontSize =
        //        request.fontSize; createParams.outlineColor = { 0,0,0,255 };// request.outlineColor;
        ...
        //        {
        //            if (imageText->GetOriginalTexelFormat() != IMCodec::TexelFormat::I_B8_G8_R8_A8 &&
        //            imageText->GetOriginalTexelFormat() != IMCodec::TexelFormat::I_R8_G8_B8_A8)
    ...
    {
        fRenderer->SetSelectionRect(
            {{selectionRect.rect.x0, selectionRect.rect.y0}, {selectionRect.rect.x1, selectionRect.rect.y1}});
        return RC_Success;
    }
    ...
    ResultCode OIV::ConverFormat(const OIV_CMD_ConvertFormat_Request& req, OIV_CMD_ConvertFormat_Response& res)
    {
        ...
        if (req.handle > 0)
        {
            ...
            if (original != nullptr)
            {
                ...
                ImageSharedPtr converted = IMUtil::ImageUtil::ConvertImageWithNormalization(
                    original, static_cast<TexelFormat>(req.format), rainbow);
    ...
    ResultCode OIV::CropImage(const OIV_CMD_CropImage_Request& request, OIV_CMD_CropImage_Response& response)
    {
        ...
        {
            LLUtils::RectI32 imageRect = {{0, 0},
                                          {static_cast<int32_t>(imageToCrop->GetWidth()),
            ...
            IMCodec::ImageSharedPtr subImage = IMUtil::ImageUtil::GetSubImage(imageToCrop, cuttedRect);
    ...
    ResultCode OIV::GetTexelInfo(const OIV_CMD_TexelInfo_Request& texel_request,
                                 OIV_CMD_TexelInfo_Response& texelresponse)
    {
        IMCodec::ImageSharedPtr image = GetImage(texel_request.handle);
        if (image != nullptr)
        {
            if (texel_request.x >= 0 && texel_request.x < image->GetWidth() && texel_request.y >= 0 &&
                texel_request.y < image->GetHeight())
            {
                ...
                memcpy(texelresponse.buffer, image->GetBufferAt(texel_request.x, texel_request.y),
                       texelresponse.size / CHAR_BIT);
    ...
    {
        return RC_NotImplemented;
        /*   LLUtils::native_string_type knownFileTypes = fImageLoader.GetKnownFileTypes();
