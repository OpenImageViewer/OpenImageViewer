...
namespace OIV
{
    LLUtils::native_string_type SelectionWorkflowPolicy::FormatSelectionSize(const LLUtils::RectI32& imageSpaceSelection)
    {
        return LLUtils::StringUtility::ToNativeString(std::to_string(imageSpaceSelection.GetWidth())) +
               LLUTILS_TEXT(" X ") +
               LLUtils::StringUtility::ToNativeString(std::to_string(imageSpaceSelection.GetHeight()));
    }
    ...
    LLUtils::PointI32 SelectionWorkflowPolicy::SnapToImagePixels(LLUtils::PointI32 clientPoint, double scale,
                                                                 LLUtils::PointF64 offset)
    {
        ...
        const auto snappedClientPoint = ViewTransformController::ImageToClient(
            static_cast<LLUtils::PointF64>(imagePoint), scale, offset);
