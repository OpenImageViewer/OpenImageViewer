...
void OIV::SetScale(Vector2 scale)
{
    fUVScale = scale;
    UpdateGpuParams();
}

Ogre::Vector2 OIV::GetTextureSize()
{
...
Ogre::Vector2 OIV::GetARFixedUVScale()
{
...
{
    Vector2 uvScaleFixed = GetARFixedUVScale();
    
    fFragmentParameters->setNamedConstant("uvScale", uvScaleFixed);
    fFragmentParameters->setNamedConstant("uvOffset", fUVOffset);
}


void OIV::TranslateOffset(Vector2 offset)
{
    SetOffset(fUVOffset + offset);
}

void OIV::SetOffset(Vector2 offset)
{
...
void OIV::RefreshScale()
{
    SetScale(fUVScale);
}


void OIV::RefreshOffset()
{
    SetOffset(fUVOffset);
}
...
void OIV::Zoom(Real amount)
{
    ...
    SetScale(uvNewScale);
    TranslateOffset((oldScale - uvNewScale) / 2);
}

void OIV::Pan(Vector2 amount)
{
    Vector2 panFactor = amount * GetScale().x;
    TranslateOffset(panFactor);
}

bool OIV::mouseMoved(const OIS::MouseEvent & arg)
{
    Zoom(-arg.state.Z.rel / 600.0);

    if (arg.state.buttonDown(OIS::MouseButtonID::MB_Left))
        Pan(-Vector2(arg.state.X.rel, arg.state.Y.rel) * 0.002);
    return false;
}
